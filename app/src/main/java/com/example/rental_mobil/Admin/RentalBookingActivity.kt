...
import com.example.rental_mobil.databinding.ActivityRentalAdminBookingBinding
import com.google.firebase.firestore.FirebaseFirestore

class RentalBookingActivity : AppCompatActivity() {
    private lateinit var b: ActivityRentalAdminBookingBinding

    val dataRental = mutableListOf<HashMap<String,String>>()
    lateinit var rentalAdp : AdapterRentalBooking

    lateinit var db: FirebaseFirestore

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        b = ActivityRentalAdminBookingBinding.inflate(layoutInflater)
        setContentView(b.root)
        supportActionBar?.setTitle("Daftar Booking Mobil")
        supportActionBar?.setDisplayHomeAsUpEnabled(true)

        rentalAdp = AdapterRentalBooking(dataRental, this)
